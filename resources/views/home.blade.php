...
<body>
  ...
  <!-- Page Content -->
    ...
    <!-- /.row -->
    <div id="ideas" class="row offset-md-2">
      <div class="col-md-12 mb-4">
       
        <a href="{{route('idea.create')}}" class="btn btn-outline-gold offset-md-4"> Emmettre une nouvelle idée</a>
       
      </div>

      @foreach ($ideas as $idea)
      <div class="idea col-md-5">
        @if(strlen($idea->topic) > 24)
        <a href="{{route('idea.show',$idea)}}"> <h4> {{substr($idea->topic, 0, 25) . '...'}} </h4> </a>  
       @else
        <a href="{{route('idea.show',$idea)}}"> <h4>{{$idea->topic}}</h4></a>
       @endif  
        <p class="mb-0 text-white-50">{{$idea->content}}</p>
        <hr class="bl mb-4">

        <center>
          <span style="color: whitesmoke">{{$idea->likes}} </span>  <a href="" class="circle like mr-2">  
            <i class="fas fa-thumbs-up"></i>
          </a> 

          <span style="color: whitesmoke">{{$idea->unlikes}}</span>  <a href="" class="circle dislike mr-2">  
